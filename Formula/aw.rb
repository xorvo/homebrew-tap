class Aw < Formula
  desc "Isolated workspaces for AI agents, with a tmux-based live dashboard"
  homepage "https://github.com/xorvo/aw"
  version "1.2.0"
  license "MIT"

  on_macos do
    on_arm do
      url "https://github.com/xorvo/aw/releases/download/v1.2.0/aw-v1.2.0-aarch64-apple-darwin.tar.gz"
      sha256 "8765a29b1b0dbf5ccd076ef8c15af11bf1b9657a36c21e40c444fe09f87e2f2c"
    end
    on_intel do
      url "https://github.com/xorvo/aw/releases/download/v1.2.0/aw-v1.2.0-x86_64-apple-darwin.tar.gz"
      sha256 "1dc995cc293e142a1725ce20d014c172a439d37d4f6327fb79336309dbd8b597"
    end
  end

  depends_on :macos
  depends_on "git"
  depends_on "tmux"

  def install
    bin.install "aw"
  end

  def caveats
    <<~EOS
      Get started:
        aw install all      # shell hook + agent hooks + tmux bindings
        aw edit-config      # configure your repos / local files
        aw init             # materialize the default base
        aw create my-task   # create a workspace
        aw dash             # open the agent dashboard

      Upgrade later: `brew upgrade aw` — or use the built-in
      `aw self update` which fetches the same release tarballs.
    EOS
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/aw --version")
  end
end
